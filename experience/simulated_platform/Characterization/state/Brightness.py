        ...
        with self.lock:
            if self._enable_decrease(self):
                ...
                if response.status_code == 200:
                    globalFrame.loc(env, "brightness_down", 'Event', self.room, 'Brightness', self.room + '.Brightness.state: ' + value, 'None', Time, self.space)
                else:
                    print("请求失败")
    ...
    def ext_action_increase(self, env):
        ...
        with self.lock:
            if self._enable_increase(self):
                ...
                response = requests.post(url)
                if response.status_code == 200:
                    globalFrame.loc(env, "brightness_up", 'Event', self.room, 'Brightness', self.room + '.Brightness.state: ' + value, 'None', Time, self.space)
                else:
                    print("请求失败")
