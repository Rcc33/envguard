        ...
        with self.lock:
            if self._enable_decrease(self):
                ...
                if response.status_code == 200:
                    print("请求成功")
                    globalFrame.loc(env, "humidity_down", 'Event', self.room, 'Humidity', self.room + '.Humidity.state: ' + value, 'Environment Change', Time, self.space)
                else:
                    print("请求失败")
    ...
    def ext_action_increase(self, env):
        ...
        with self.lock:
            if self._enable_increase(self):
                ...
                if response.status_code == 200:
                    print("请求成功")
                    globalFrame.loc(env, "humidity_up", 'Event', self.room, 'Humidity', self.room + '.Humidity.state: ' + value, 'Environment Change', Time, self.space)
                else:
                    print("请求失败")
