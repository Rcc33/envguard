...
class Speaker(BaseType):
    ...
    def action_on(self, env, source): 
        ...
        if self._enable_on(self):
            ...
            url = getIP() + "/set_device_value/" + self.room_name + "/Speaker001/1"
            response = requests.post(url)
            if response.status_code == 200:
                self.lock.release()  
                Time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(globalFrame.start_time))
                globalFrame.loc(env, "speaker_on", 'Action', self.room_name, 'Speaker', self.room_name + '.Speaker.state: 1', source, Time, self.space)
    ...
    def _effect(self, env, expect_value, action_name, source):
        url = getIP() + "/effect_and_pre/" + self.room_name + "/Speater001/" + action_name
        ...
        if self.ap_on(self) == expect_value:
            ...
            self.lock.release()
        if expect_value == '1':
            time.sleep(random.uniform(0.05, 1.1))
    ...
    def action_off(self, env, source): 
        ...
        if self._enable_off(self):
            url = getIP() + "/set_device_value/" + self.room_name + "/Speaker001/0"
            response = requests.post(url)
            if response.status_code == 200:
                self.lock.release()
                Time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(globalFrame.start_time))
                globalFrame.loc(env, "speaker_off", 'Action', self.room_name, 'Speaker', self.room_name + '.Speaker.state: 0', source, Time, self.space)
    ...
    def ap_on(self):
        url = getIP() + "/room_device_info/" + self.room_name + "/Speaker001"
