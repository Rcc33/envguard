...
LTL = {
  "G !(Lab.temperature_up & Lab.temperature_down)",
  "G !(TeaRoom.temperature_up & TeaRoom.temperature_down)",
  "G !(MeetingRoomOne.temperature_up & MeetingRoomOne.temperature_down)",
  "G !(MeetingRoomTwo.temperature_up & MeetingRoomTwo.temperature_down)",
  
  
  "G(Lab.Brightness.high & Lab.HumanState.detected)",
  
  
  "G !(Context.Weather.raining & Lab.Window.on)",
  "G !(Context.Weather.raining & TeaRoom.Window.on)",
  "G !(Context.Weather.raining & MeetingRoomOne.Window.on)",
  "G !(Context.Weather.raining & MeetingRoomTwo.Window.on)",
  # "G !(Context.Weather.raining & Corridor.Window.on)",
  
  # "G (Lab.brightness_up & (Lab.Temperature.high))"
}

EventState = {
  "G (TeaRoom.human_undetected & !(TeaRoom.MicrowaveOven.on))",
  "G (TeaRoom.human_undetected & !(TeaRoom.WaterDispenser.on))",
}
...
MTL = [
  ...
  # "G(Lab.human_detected -> (F[0, 30] Lab.Brightness.high))",
  ...
  "G((TeaRoom.WaterDispenser.on) -> (F[0, 2*60] !(TeaRoom.WaterDispenser.off)))",
...
def getLTL(list):
  result_list = []
  url = "http://47.101.169.122:5004/effect_node/" + list[0]+ '/effect_' + list[1]
  ...
  if not flag:
    if type in device_list:
      ...
      if type == 'HumanState':
        ...
        if value == 'high':
          action_list = []
          url = "http://47.101.169.122:5004/effect_node/" + room+ '/effect_' + toSnake(type) + '_up'
          results = requests.get(url).json()
          for result in results:
            ltl_action = ''
            action_item = result['room'] + '.' + result['device'][0:len(result['device']) - 3] + '.' + result['action'][7:len(result['action'])]
          ...
          if '(' + "'" + toSnake(type) + '_up' + "'" + '==' + 'df.iat[index, 0]' + ' and '  + "'" + room  + "'" + '==' + 'df.iat[index, 2]' + ')' not in actions:
            if actions:
              ...
              actions = '(' + "'" + toSnake(type) + '_up' + "'" + '==' + 'df.iat[index, 0]' + ' and '  + "'" + room  + "'" + '==' + 'df.iat[index, 2]' + ')'
          action_list = []
          url = "http://47.101.169.122:5004/effect_node/" + room+ '/effect_' + toSnake(type) + '_down'
          results = requests.get(url).json()
          for result in results:
            ltl_action = ''
            action_item = result['room'] + '.' + result['device'][0:len(result['device']) - 3] + '.' + result['action'][7:len(result['action'])]
    ...
    if count == 0:
      ...
      for state in states.split('&'):
        ...
        if trigger:
          ...
          trigger = f
      url = "http://47.101.169.122:5004/effect_node/" + effect.split('.')[0]+ '/effect_' + effect.split('.')[1]
      results = requests.get(url).json()
      fix_temp = []
      for result in results:
