...
class Door(BaseType):
    ...
    def _effect(self, env, expect_value, action_name):
        url = getIP() + "/effect_and_pre/" + self.room_name + "/Door001/" + action_name
        effect_and_pre = requests.get(url).json()
        self.lock.acquire()
        if self.ap_on(self) == expect_value:
            self.lock.release()
            time.sleep(random.uniform(0.6*8, 0.6*10))
            self.lock.acquire()
            if self.ap_on(self) == expect_value:
                self.lock.release()
    ...
    def action_on(self, env, source): 
        ...
        if self._enable_on(self):
            ...
            url = getIP() + "/set_device_value/" + self.room_name + "/Door001/1"
            response = requests.post(url)
            if response.status_code == 200:
                self.lock.release()
                Time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(globalFrame.start_time))
                globalFrame.loc(env, "door_on", 'Action', self.room_name, 'Door', self.room_name + '.Door.state: 1', source, Time, self.space)
    ...
    def action_off(self, env, source): 
        ...
        if self._enable_off(self):
            # 判断pre-conditions，产生相应的effects 
            url = getIP() + "/set_device_value/" + self.room_name + "/Door001/0"
            response = requests.post(url)
            if response.status_code == 200:
                self.lock.release()
                Time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(globalFrame.start_time))
                globalFrame.loc(env, "door_off", 'Action', self.room_name, 'Door', self.room_name + '.Door.state: 0', source, Time, self.space)
    ...
    def ap_on(self):
        url = getIP() + "/room_device_info/" + self.room_name + "/Door001"
