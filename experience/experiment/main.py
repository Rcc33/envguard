...
def check(regulation, indexs):
    ...
    if regulation['flag'] == 'F':
        ...
        if regulation['contact'] == 'or':
            ...
            if len(fix_list) == 1 and fix_list[0] == undo:
                if df.iat[line-1, 9]:
                    df.iat[line-1, 9] = df.iat[line-1, 9] + ', ' + 'Undo'
                else:
                    df.iat[line-1, 9] = 'Undo'
                if df.iat[line-1, 8]:
                    df.iat[line-1, 8] = df.iat[line-1, 8] + ', ' + '(' + undo + ')'
                else:
                    df.iat[line-1, 8] = '(' + undo + ')'
            else:
                if df.iat[line-1, 9]:
                    df.iat[line-1, 9] = df.iat[line-1, 9] + ', ' + 'Repair'
                else:
                    df.iat[line-1, 9] = 'Repair'
                if df.iat[line-1, 8]:
                    df.iat[line-1, 8] = df.iat[line-1, 8] + ', ' + '(' + ' or '.join(fix_list)+ ')'
                else:
                    df.iat[line-1, 8] = '(' + ' or '.join(fix_list) + ')'
        if df.iat[line-1, 7]:
            df.iat[line-1, 7] = df.iat[line-1, 7] + '; ' + regulation['mtl']
        else:
            df.iat[line-1, 7] = regulation['mtl']
        # global end
        # end = datetime.datetime.now() 
        # print('修复：', end - start)  
    # print()          
