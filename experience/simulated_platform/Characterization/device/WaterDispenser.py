...
class WaterDispenser(BaseType):
    ...
    def action_on(self, env, source): 
        ...
        if self._enable_on(self):
            ...
            url = getIP() + "/set_device_value/" + self.room_name + "/WaterDispenser001/1"
            response = requests.post(url)
            if response.status_code == 200:
                self.lock.release()
                Time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(globalFrame.start_time))
                globalFrame.loc(env, "water_dispenser_on", 'Action', self.room_name, 'WaterDispenser', self.room_name + '.WaterDispenser.state: 1', source, Time, self.space)
    ...
    def _effect(self, env, expect_value, action_name, source):
        url = getIP() + "/effect_and_pre/" + self.room_name + "/WaterDispenser001/" + action_name
        ...
        if self.ap_on(self) == expect_value:
            ...
            self.lock.release()    
        if expect_value == '1':
            time.sleep(random.uniform(0.3, 0.6*1))
    ...
    def action_off(self, env, source): 
        ...
        if self._enable_off(self):
            url = getIP() + "/set_device_value/" + self.room_name + "/WaterDispenser001/0"
            response = requests.post(url)
            if response.status_code == 200:
                self.lock.release()
                Time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(globalFrame.start_time))
                globalFrame.loc(env, "water_dispenser_off", 'Action', self.room_name, 'WaterDispenser', self.room_name + '.WaterDispenser.state: 0', source, Time, self.space)
    ...
    def ap_on(self):
        url = getIP() + "/room_device_info/" + self.room_name + "/WaterDispenser001"
