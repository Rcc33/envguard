        ...
        with self.lock:
            if self._enable_decrease(self):
                ...
                if response.status_code == 200:
                    globalFrame.loc(env, "noise_down", 'Event', self.room, 'Noise', self.room + '.Noise.state: ' + value, 'None', Time, self.space)
                else:
                    print("请求失败")
    ...
    def ext_action_increase(self, env):
        ...
        with self.lock:
            if self._enable_increase(self):
                ...
                response = requests.post(url)
                if response.status_code == 200:
                    globalFrame.loc(env, "noise_up", 'Event', self.room, 'Noise', self.room + '.Noise.state: ' + value, 'None', Time, self.space)
                else:
                    print("请求失败")
