...
class Printer(BaseType):
    ...
    def action_on(self, env, source): 
        ...
        if self._enable_on(self):
            ...
            url = getIP() + "/set_device_value/" + self.room_name + "/Printer001/1"
            ...
            if response.status_code == 200:
                ...
                globalFrame.loc(env, "printer_on", 'Action', self.room_name, 'Printer', self.room_name + '.Printer.state: 1', source, Time, self.space)    
    ...
    def _effect(self, env, expect_value, action_name, source):
        url = getIP() + "/effect_and_pre/" + self.room_name + "/Printer001/" + action_name
    ...
    def action_off(self, env, source): 
        ...
        if self._enable_off(self):
            url = getIP() + "/set_device_value/" + self.room_name + "/Printer001/0"
            response = requests.post(url)
            if response.status_code == 200:
                print("请求成功")
                self.lock.release() 
                ...
                Time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(globalFrame.start_time))
                globalFrame.loc(env, "printer_off", 'Action', self.room_name, 'Printer', self.room_name + '.Printer.state: 0', source, Time, self.space)
    ...
    def ap_on(self):
        url = getIP() + "/room_device_info/" + self.room_name + "/Printer001"
