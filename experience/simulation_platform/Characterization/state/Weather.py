        ...
        with self.lock:
            if self._enable_change(self, value):
                ...
                if response.status_code == 200:
                    print("请求成功")
                    print(self.room + '.Weather.state: ' + value)
                    globalFrame.loc(env, "weather_change", 'Event', self.room, 'Weather', self.room + '.Weather.state: ' + value, 'Environment Change', Time, self.space)
                else:
                    print("请求失败")
